...
实际光源通常是准单色的，即：
U(**r**, t) = U₀(**r**, t) exp(-iω₀t)

其中 U₀(**r**, t) 是缓变包络（相对于载波频率 ω₀），满足：
|∂U₀/∂t| ≪ ω₀|U₀|
...
U⁺(t) = (1/2π) ∫₀^∞ Ũ(ω) exp(-iωt) dω

其中 Ũ(ω) 是实场的傅里叶变换。解析信号的实部给出物理场：
E(t) = 2Re[U⁺(t)]
...
归一化的互强度定义为复相干度：
γ(**r₁**, **r₂**, τ) = J(**r₁**, **r₂**, τ) / √[I(**r₁**)I(**r₂**)]

其模值 |γ| 描述相干程度，相位 arg(γ) 描述两点间的平均相位差。
...
时域互强度的傅里叶变换给出交叉谱密度：
W(**r₁**, **r₂**, ω) = ∫ J(**r₁**, **r₂**, τ) exp(iωτ) dτ

它描述了不同频率分量的空间相干性，满足：
J(**r₁**, **r₂**, τ) = (1/2π) ∫ W(**r₁**, **r₂**, ω) exp(-iωτ) dω

**相干模式分解**：

互强度可分解为相干模式的叠加：
J(**r₁**, **r₂**) = ∑ₙ λₙ ψₙ*(**r₁**)ψₙ(**r₂**)

其中 λₙ 是特征值，ψₙ(**r**) 是正交模式，满足：
∫ J(**r**, **r'**)ψₙ(**r'**) d**r'** = λₙ ψₙ(**r**)
...
其中 d 是探测器间距。当 d 增大时，|γ⁽¹⁾(d)| 下降，通过测量 g⁽²⁾ 的空间分布可推断源的角尺寸：
θ ≈ λ/d₀

其中 d₀ 是 g⁽²⁾ 降到 1.5 时的间距。
...
γ₁₂ = ∫∫ I(ξ, η) exp[ik(ξx₁₂ + ηy₁₂)/z] dξ dη / ∫∫ I(ξ, η) dξ dη

其中 (ξ, η) 是源坐标，x₁₂ = x₂ - x₁，y₁₂ = y₂ - y₁。

这个定理的重要推论：
- 均匀圆形源：γ(ρ) = 2J₁(kDρ/2z)/(kDρ/2z)
- 均匀矩形源：γ(x, y) = sinc(kDₓx/z) × sinc(kDᵧy/z)
...
其中 K 是传播核（Green函数）：
K(P, Q) = (exp(ikr)/iλr) × (1 + cos θ)/2

对于傍轴近似：
K(P, Q) ≈ (exp(ikz)/iλz) exp[ik|**r**ₚ - **r**ᵩ|²/2z]
